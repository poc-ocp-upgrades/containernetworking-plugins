...
var _ = Describe("base functionality", func() {
	...
	It("Works with a valid config", func() {
		...
		args := &skel.CmdArgs{ContainerID: "dummy", Netns: targetNS.Path(), IfName: CNI_IFNAME, StdinData: []byte(conf)}
		var resI types.Result
		err = originalNS.Do(func(ns.NetNS) error {
			defer GinkgoRecover()
			var err error
			resI, _, err = testutils.CmdAddWithResult(targetNS.Path(), CNI_IFNAME, []byte(conf), func() error {
				return cmdAdd(args)
			})
			return err
		})
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(res.Interfaces).To(Equal([]*current.Interface{{Name: CNI_IFNAME, Mac: origLink.Attrs().HardwareAddr.String(), Sandbox: targetNS.Path()}}))
		...
		err = originalNS.Do(func(ns.NetNS) error {
			defer GinkgoRecover()
			err = testutils.CmdDelWithResult(targetNS.Path(), CNI_IFNAME, func() error {
				return cmdDel(args)
			})
			Expect(err).NotTo(HaveOccurred())
			_, err := netlink.LinkByName(ifname)
	...
	It("fails an invalid config", func() {
		...
		args := &skel.CmdArgs{ContainerID: "dummy", Netns: originalNS.Path(), IfName: ifname, StdinData: []byte(conf)}
		_, _, err := testutils.CmdAddWithResult(originalNS.Path(), ifname, []byte(conf), func() error {
			return cmdAdd(args)
		})
