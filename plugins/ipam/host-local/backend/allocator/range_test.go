...
var _ = Describe("IP ranges", func() {
	It("should generate sane defaults for ipv4", func() {
		...
		Expect(r).To(Equal(Range{Subnet: mustSubnet(snstr), RangeStart: net.IP{192, 0, 2, 1}, RangeEnd: net.IP{192, 0, 2, 254}, Gateway: net.IP{192, 0, 2, 1}}))
	})
	It("should generate sane defaults for a smaller ipv4 subnet", func() {
		...
		err := r.Canonicalize()
		Expect(err).NotTo(HaveOccurred())
		Expect(r).To(Equal(Range{Subnet: mustSubnet(snstr), RangeStart: net.IP{192, 0, 2, 1}, RangeEnd: net.IP{192, 0, 2, 126}, Gateway: net.IP{192, 0, 2, 1}}))
	})
	It("should generate sane defaults for ipv6", func() {
		...
		err := r.Canonicalize()
		Expect(err).NotTo(HaveOccurred())
		Expect(r).To(Equal(Range{Subnet: mustSubnet(snstr), RangeStart: net.ParseIP("2001:DB8:1::1"), RangeEnd: net.ParseIP("2001:DB8:1::ffff:ffff:ffff:ffff"), Gateway: net.ParseIP("2001:DB8:1::1")}))
	})
	...
	It("should reject invalid RangeStart and RangeEnd specifications", func() {
		...
		r = Range{Subnet: mustSubnet("192.0.2.0/24"), RangeStart: net.ParseIP("192.0.2.50"), RangeEnd: net.ParseIP("192.0.2.40")}
		err = r.Canonicalize()
	...
	It("should parse all fields correctly", func() {
		r := Range{Subnet: mustSubnet("192.0.2.0/24"), RangeStart: net.ParseIP("192.0.2.40"), RangeEnd: net.ParseIP("192.0.2.50"), Gateway: net.ParseIP("192.0.2.254")}
		err := r.Canonicalize()
		Expect(err).NotTo(HaveOccurred())
		Expect(r).To(Equal(Range{Subnet: mustSubnet("192.0.2.0/24"), RangeStart: net.IP{192, 0, 2, 40}, RangeEnd: net.IP{192, 0, 2, 50}, Gateway: net.IP{192, 0, 2, 254}}))
	})
	...
	It("should accept v6 IPs in range and reject IPs out of range", func() {
		r := Range{Subnet: mustSubnet("2001:DB8:1::/64"), RangeStart: net.ParseIP("2001:db8:1::40"), RangeEnd: net.ParseIP("2001:db8:1::50")}
		err := r.Canonicalize()
		Expect(err).NotTo(HaveOccurred())
	...
	DescribeTable("Detecting overlap", func(r1 Range, r2 Range, expected bool) {
		r1.Canonicalize()
		r2.Canonicalize()
		Expect(r1.Overlaps(&r2)).To(Equal(expected))
		Expect(r2.Overlaps(&r1)).To(Equal(expected))
	}, Entry("non-overlapping", Range{Subnet: mustSubnet("10.0.0.0/24")}, Range{Subnet: mustSubnet("10.0.1.0/24")}, false), Entry("different families", Range{Subnet: mustSubnet("0.0.0.0/24")}, Range{Subnet: mustSubnet("::/24")}, false), Entry("Identical", Range{Subnet: mustSubnet("10.0.0.0/24")}, Range{Subnet: mustSubnet("10.0.0.0/24")}, true), Entry("Containing", Range{Subnet: mustSubnet("10.0.0.0/20")}, Range{Subnet: mustSubnet("10.0.1.0/24")}, true), Entry("same subnet, non overlapping start + end", Range{Subnet: mustSubnet("10.0.0.0/24"), RangeEnd: net.ParseIP("10.0.0.127")}, Range{Subnet: mustSubnet("10.0.0.0/24"), RangeStart: net.ParseIP("10.0.0.128")}, false), Entry("same subnet, overlapping start + end", Range{Subnet: mustSubnet("10.0.0.0/24"), RangeEnd: net.ParseIP("10.0.0.127")}, Range{Subnet: mustSubnet("10.0.0.0/24"), RangeStart: net.ParseIP("10.0.0.127")}, true))
})
